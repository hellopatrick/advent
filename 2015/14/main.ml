...
module Position = struct
  ...
  type t = { reindeer:Reindeer.t; state:state; time_in_state:int; flight_distance:int}

  let compare a b =
    Int.compare a.flight_distance b.flight_distance

  let create reindeer =
    {reindeer; state=Flying; time_in_state=0; flight_distance=0}
...
let race reindeers n =
  let rec aux positions n =
    ...
    else aux (List.map positions ~f:Position.update) (n-1)
  in
...
let _ =
  ...
  match List.max_elt results ~cmp:Position.compare with
  | Some winner -> printf "%s won with %d km!" winner.reindeer.name winner.flight_distance
  | None -> printf "No one won."